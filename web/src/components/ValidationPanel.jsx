...
const ValidationPanel = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    const labels = {
      ...
      [StatusValidacao.EM_DILIGENCIA]: "Em Diligência",
        ...
        <CardContent className="pt-4">
          <div className="flex items-center justify-between">
              ...
              <div>
                ...
                {operacoes[dataAprovacao] && (
                  <p className="text-sm text-gray-500">
                    Aprovado em:{" "}
                    {new Date(operacoes[dataAprovacao]).toLocaleDateString(
                      "pt-BR"
                    )}
                  </p>
                )}
            ...
            <div className="flex space-x-2">
              {/* Botão para abrir/fechar Salas */}
              <Button
                size="sm"
                variant="outline"
                onClick={() => setShowSalas(!showSalas)}
              >
                {showSalas ? "Ocultar Salas" : "Exibir Salas"}
              </Button>
              ...
              <Button
                ...
                className={
                  operacoes[campo] ? "bg-green-600 hover:bg-green-700" : ""
                }
              >
                Aprovar
              </Button>
              <Button
                ...
                onClick={() =>
                  handleOperacaoChange(campo, false, dataAprovacao)
                }
              >
                Rejeitar
              </Button>
          ...
          {/* Aqui entra o conteúdo das salas */}
          {showSalas && (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        {[sala.foto1, sala.foto2, sala.foto3]
                          .filter(Boolean)
                          .map((foto, index) => (
                            <div
                              key={index}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            handleObservacaoChange(sala.id, e.target.value)
                          }
                          className="mt-1"
                          rows={3}
                    ...
                    <div className="flex space-x-2 pt-4 border-t">
                      <Button
                        onClick={() =>
                          handleSalaStatusChange(
                            sala.id,
                            StatusValidacao.APROVADO,
                            sala.observacaoAvaliador
                          )
                        }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Avalie e valide documentos e instalações do CCPS
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {ccpsData.endereco}, {ccpsData.cidade}, {ccpsData.estado}
              </p>
            </div>
            <div>
              ...
              <p className="text-sm text-gray-600">
                {ccpsData.veterinario.nome} - {ccpsData.veterinario.crmv}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="salas" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Progresso das Validações</CardTitle>
            </CardHeader>
            <CardContent>
              {(() => {
                const totalOperacoes = 6; // número de OperacaoItem que você tem
                const aprovadas = [
                  "arquivosProcessosTecnologicos",
                  "fluxoOperacionalDefinido",
                  "medidasHigienicoSanitariasFuncionarios",
                  "medidasHigienicoSanitariasVisitantes",
                  "controlePragas",
                  "sistemaEscoamento",
                ].filter((campo) => operacoes[campo]).length;

                const progresso = (aprovadas / totalOperacoes) * 100;

                return (
                  <div className="space-y-2">
                    <p className="text-sm text-gray-700">
                      {aprovadas} de {totalOperacoes} operações aprovadas
                    </p>
                    <Progress value={progresso} className="h-4 rounded" />
                  </div>
                );
              })()}
            </CardContent>
